...
RSpec.describe Task, type: :model do
  ...
  describe 'Validation test' do
    ...
    context 'If the task description is empty' do
      it 'Validation fails' do
        task = Task.create(title: 'Test1', content: '', deadline_on: '2023/5/25', priority: 'low', status: 'done', user_id: user.id)
        expect(task).not_to be_valid
    ...
    context 'If the task Title and description have values' do
      it 'You can register a task' do
        task = Task.create(title: 'Titre2', content: 'Create a proposal.', deadline_on: '2023/5/25', priority: 'low', status: 'done', user_id: user.id)
  ...
  describe 'Fonction de recherche' do
    ...
    let!(:first_task) { FactoryBot.create(:task, title: 'first_task_title', deadline_on: '2025/02/18', priority: 'medium', status: 'todo', user: user) }
    let!(:second_task) { FactoryBot.create(:second_task, title: "second_task_title", deadline_on: '2025/02/17', priority: 'high', status: 'doing', user: user) }
    let!(:third_task) { FactoryBot.create(:third_task, title: "third_task_title", deadline_on: '2025/02/16', priority: 'low', status: 'done', user: user) }
